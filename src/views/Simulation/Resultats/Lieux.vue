<template>
  <div class="aj-unbox">
    <button class="aj-etablissements-back-button button outline small with-icon" type="button" v-on:click="window && window.history.back()">
      ...
      {{ isGeneric ? 'Retour aux résultats' : 'Retour à l\'aide'}}
    </button>
    <p class="aj-etablissements-intro">
        {{ isGeneric ? 'Voici les lieux où vous pouvez être accompagné(e) pour faire vos demandes et poser toutes vos questions.' : 'Vous pouvez y être accompagné·e pour faire votre demande et poser toutes vos questions.'}}
    </p>
...
export default {
  ...
  computed: {
    isGeneric() {
        return this.$route.params.id === 'generic'
    }
  },
  mounted: function() {
